...
@section('page-subtitle', 'Update segment conditions')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-7">
        ...
        <div>
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Segment</h2>
            @if(isset($eligibleCount))
                <p class="text-sm text-slate-500 mt-0.5">
                    <span class="text-emerald-600 font-semibold">{{ number_format($eligibleCount) }}</span> eligible contacts match current conditions
                </p>
            @endif
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('segments.update', $segment) }}" x-data="segmentBuilder()">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $segment->name) }}" required
            ...
            <div>
                ...
                <input type="text" name="description" value="{{ old('description', $segment->description) }}"
                       class="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
            </div>
...
function segmentBuilder() {
    return {
        conditions: @json(old('conditions', $segment->conditions ?? [])),
        addCondition() {
