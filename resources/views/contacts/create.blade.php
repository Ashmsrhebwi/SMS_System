...
@section('page-subtitle', 'Manually add a new contact to your list')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-7">
        ...
        <div>
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Add Contact</h2>
            <p class="text-sm text-slate-500 mt-0.5">Fill in the details below to add a new contact.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('contacts.store') }}">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-5">
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-slate-400">International format recommended, e.g. +447700900000</p>
            </div>
            ...
            <div>
                ...
                <textarea id="notes" name="notes" rows="3"
                          class="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-shadow resize-none"
                          placeholder="Any notes about this contact…">{{ old('notes') }}</textarea>
            </div>

            @if($tags->count())
            <div>
                <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Tags <span class="text-slate-400 font-normal">(optional)</span></label>
            ...
            <div class="flex items-center justify-between pt-1">
                <label class="inline-flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="opted_in" value="1" {{ old('opted_in', '1') ? 'checked' : '' }}
                           class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
